...
describe('Edge Cases', () => {
  ...
  it('add branch with negative weight throws', () => {
    const branch = pb();
    expect(() => branch.add(-1, () => 1)).toThrow();
  });

  it('add branch with non-number weight throws', () => {
    const branch = pb();
    // @ts-expect-error
    expect(() => branch.add('a', () => 1)).toThrow();
  });

  it('add branch with non-function handler throws', () => {
    const branch = pb();
    // @ts-expect-error
    expect(() => branch.add(1, 123)).toThrow();
  });
  ...
  it('limit=0 allows unlimited runs', () => {
    const branch = pb({ limit: 0 }).add(1, () => 1);
